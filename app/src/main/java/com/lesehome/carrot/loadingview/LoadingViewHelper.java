...
public class LoadingViewHelper<DATA> {
    ...
    public LoadingViewHelper(IRefreshView refreshView) {
        this(refreshView, loadViewFactory.madeLoadView(), loadViewFactory.madeLoadMoreView());
    }

    public LoadingViewHelper(IRefreshView refreshView, ILoadViewFactory.ILoadView loadView) {
        this(refreshView, loadView, null);
    }

    public LoadingViewHelper(IRefreshView refreshView, ILoadViewFactory.ILoadView loadView, ILoadViewFactory.ILoadMoreView loadMoreView) {
        super();
    ...
    private class RefreshAsyncTask extends MyAsyncTask<Void, Void, DATA> {
        ...
        protected void onCancelled() {
            super.onCancelled();
            if (handler != null && showRefreshing != null) {
                handler.removeCallbacks(showRefreshing);
            }
        }
    ...
    private OnScrollBottomListener onScrollBottomListener = new OnScrollBottomListener() {
        ...
        public void onScorllBootom() {
            if (autoLoadMore && hasMoreData && !isLoading()) {
                // 如果网络可以用
                if (needCheckNetwork && !hasNetwork(context)) {
     ...
     * @return
     */
    private boolean hasNetwork(Context paramContext) {
        try {
